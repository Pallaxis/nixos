{pkgs, ...}: let
  toggle-workspace-number = pkgs.writeShellApplication {
    name = "toggle-workspace-number";
    text = ''
      workspace_pressed=$1
      ...
      if [[ "$current_active_workspace" == "$workspace_pressed" ]]; then
          hyprctl dispatch "hl.dsp.focus({{ workspace = $previous_active_workspace })"
      else
          hyprctl dispatch "hl.dsp.focus({ workspace = $workspace_pressed })"
      fi
      echo "$current_active_workspace" | tee /tmp/previous_workspace > /dev/null
    '';
  };
in {
  home.packages = [toggle-workspace-number];
}
