...
local monitor_configs = {
  ...
  night = {
    primary = { output = "desc:Ancor Communications Inc ROG PG278Q #ASMObu8P/tXd", mode = "2560x1440@144", position = "-2560x0", scale = "1" },
    secondary = { output = "desc:Samsung Electric Company LC32G7xT H4ZR900653", mode = "2560x1440@144", position = "0x0", scale = "1" },
  },
}
...
hl.on("hyprland.start", function()
  ...
  hl.exec_cmd(Term, { workspace = "2 silent" })
  if Hostname == "thinkpad" then
    hl.exec_cmd("slack", { workspace = "5 silent" })
  end
end)
...
hl.bind("SUPER + SHIFT + 0", hl.dsp.window.move({ workspace = 10, follow = true }), { description = "Move focused window to workspace 10" })
...
hl.bind("SUPER + ALT + 0", hl.dsp.window.move({ workspace = 10, follow = false }), { description = "Move focused window to workspace 10 silently" })
...
hl.window_rule({ match = { class = "^(ffplay)$" }, float = true, no_initial_focus = true })
